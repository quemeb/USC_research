...
pattern_dist = re.compile(r"dist=(\d+)", re.IGNORECASE)
...
def extract_dist(lists):
    # Extract all numbers, convert them to integers, and flatten the list
    return [int(num) for lst in lists for num in pattern_dist.findall(lst)]
...
def process_all_files():
    ...
    for filename in glob.glob(path):
        ...
        with open(distance_filename, 'w') as f:
            for distance in inter_distance:
                f.write(f"{distance}\n")
